...
class ActionDatabase::Populator {
    ...
    template <Action a>
    struct ActionNamespace<a, std::enable_if_t<std::is_same_v<typename ActionProperties<a>::app_namespace, std::true_type>, void>> {
    ...
    struct Accelerators<a, std::void_t<decltype(&ActionProperties<a>::accelerators)>> {
        ...
        static_assert(std::is_same_v<std::remove_extent_t<decltype(ActionProperties<a>::accelerators)>, const char* const>);
        static_assert(ActionProperties<a>::accelerators[std::extent_v<decltype(ActionProperties<a>::accelerators)> - 1] == nullptr,
    ...
    template <Action a, std::enable_if_t<has_param<a>() && has_state<a>(), bool> = true>
    static void assign(ActionDatabase* db) {
        static_assert(std::is_same_v<typename ActionProperties<a>::state_type, typename ActionProperties<a>::parameter_type>);
        ACTIONDB_PRINT_DEBUG(START_ROW << " | \""
                                       << (const char*)gVariantType<typename ActionProperties<a>::state_type>()
